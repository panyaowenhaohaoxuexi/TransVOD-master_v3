...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # Always save checkpoint.pth (overwrite) for resuming.
        # Also save checkpoint{epoch}.pth first, then delete it if not best.
        epoch_ckpt_path = None
        if args.output_dir:
            # Always keep one rolling checkpoint
            utils.save_on_master({
                'model': model_without_ddp.state_dict(),
                'optimizer': optimizer.state_dict(),
                'lr_scheduler': lr_scheduler.state_dict(),
            ...
            # Save epoch-specific checkpoint (candidate)
        ...
        if (key_metric is not None) and args.output_dir and utils.is_main_process() and (key_metric > best_metric):
            ...
            # delete previous best epoch checkpoint to save space (optional but usually desired)
            ...
            # optional: save evaluator outputs for best
            try:
