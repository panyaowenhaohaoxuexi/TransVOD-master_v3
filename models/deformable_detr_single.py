...
class DeformableDETR(nn.Module):
    ...
    def _split_trimodal_single(self, samples: NestedTensor):
        """
        samples.tensors: [B, 9, H, W] -> (vis, ir, sar), each is NestedTensor([B,3,H,W])
        """
        x, mask = samples.tensors, samples.mask
        assert x.dim() == 4 and x.size(1) == 9, f"expect [B,9,H,W], got {list(x.shape)}"
        vis = x[:, 0:3]
        ir  = x[:, 3:6]
        sar = x[:, 6:9]
        return NestedTensor(vis, mask), NestedTensor(ir, mask), NestedTensor(sar, mask)

    def _backbone_to_transformer_inputs(self, samples_mod: NestedTensor):
        """
        Run backbone + input_proj and build (srcs, masks, pos) for transformer.
        """
        features, pos = self.backbone(samples_mod)
        ...
        # extra feature levels (same as original logic)
